...
Given a singly linked list, determine whether it contains a cycle. A cycle occurs when a WSDnode’s `next` pointer points back to a previous node in the list, forming a loop.
...
3. While `fast` is not `null` AND [`fast.next`](http://fast.next) is not `null`:
